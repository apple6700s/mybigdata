...
public class AsyncTaskMonitor {
    ...
    /**
     * 爬虫组给代码逻辑，如何通过一个jobName或多余的字段判断当前jobName是否需要记录progress。
     *
     * @param jsonObject
     * @return
     */
    public static boolean needRecordProgress(JSONObject jsonObject) {
        ...
        try {
            if (jsonObject.containsKey("_track_count_"))
                ...
                return (jsonObject.containsKey("info") && jsonObject.getJSONObject("info").getBoolean("_track_count_"));
        } catch (Exception e) {
            return false;
        }
    }
    ...
    /**
     * 其实应该是jobName，taskId是静态的，jobName是动态唯一的。
     * @param jsonObject
     * @return
     */
