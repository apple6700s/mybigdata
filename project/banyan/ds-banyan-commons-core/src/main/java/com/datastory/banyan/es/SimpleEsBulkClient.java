...
public class SimpleEsBulkClient implements DataSinkWriter {
    protected static Logger LOG = Logger.getLogger(SimpleEsBulkClient.class);

    protected String indexName = null;
    protected String indexType = null;
    protected String[] esHosts = new String[]{};
    ...
    protected Function<Void, TransportClient> clientBuilder = null;
    protected TransportClient client;
    protected ReadWriteLock clientLock = new ReentrantReadWriteLock();
    ...
    protected SimpleEsBulkClient(final Settings settings, String indexName, String indexType,
                                 final String[] esHosts, int bulkActions) {
        this.indexName = indexName;
        this.indexType = indexType;
        this.bulkActions = bulkActions;
        this.esHosts = esHosts;

        clientBuilder = new Function<Void, TransportClient>() {
            @Nullable
            @Override
            public TransportClient apply(@Nullable Void aVoid) {
                String[] esHosts_ = BanyanTypeUtil.shuffleCopyArray(esHosts);
                TransportAddress[] transportAddresses = new InetSocketTransportAddress[esHosts.length];
                for (int i = 0; i < esHosts_.length; i++) {
                    String[] parts = esHosts_[i].split(":");
                    int port = 9300;
                    if (parts.length > 1)
        ...
        client = clientBuilder.apply(null);

//        this.getHooks().add(getMainHook());
    }

    public void reconnectClientIfNeeded() {
        if (client == null || client.connectedNodes().size() <= esHosts.length / 2) {
            clientLock.writeLock().lock();
            int restSize = client.connectedNodes().size();
            if (client == null || restSize <= esHosts.length / 2) {
                LOG.info("[ES] Reinit transport client, connected nodes : " + restSize + " / " + esHosts.length);
                LOG.error("[ES] Reinit transport client, connected nodes : " + restSize + " / " + esHosts.length);
                client.close();
                client = clientBuilder.apply(null);
            }
    ...
    public void flush(final boolean await) {
        if (!bulkRequest.requests().isEmpty()) {
            ...
            synchronized (this) {
                if ((bulkRequest.requests().size() > bulkActions)
                        || (!bulkRequest.requests().isEmpty() && status.get()<0)) {
