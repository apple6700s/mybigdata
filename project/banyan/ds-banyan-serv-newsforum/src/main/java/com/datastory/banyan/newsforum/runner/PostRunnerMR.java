 ...
 */
public class PostRunnerMR implements Serializable {
    static final byte[] R = "r".getBytes();

    static Log LOG = LogFactory.getLog(PostRunnerMR.class);
    ...
    public static class FixMapper extends TableMapper<NullWritable, NullWritable> {
        ...
        protected void map(ImmutableBytesWritable key, Result result, Context context) throws IOException, InterruptedException {
            ...
            try {
                ...
                Params m_map = new MResultDocMapper(result).map();
                long cost = (System.currentTimeMillis() - start);
                context.getCounter(ROW.PARSE_TIME).increment(cost);
                LOG.info("[Params]:" + map.toJson());

                start = System.currentTimeMillis();
                ValidResult validResult = engine.execute(map, "post");
                ValidResult validResult1 = engine.execute(m_map, "post");
