...
"$JAVA" -Dio.netty.leakDetectionLevel=advanced -Dds.commons.logging.procName=${CLASS} -Dhdp.version=2.4.2.0-258 -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp/oom_dump/${CLASS} -Djava.awt.headless=true ${HEAP_OPTS} -classpath "$CLASSPATH" ${CLASS} ${params}
