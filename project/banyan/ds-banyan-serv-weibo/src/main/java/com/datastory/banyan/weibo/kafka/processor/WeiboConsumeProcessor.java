...
public class WeiboConsumeProcessor extends CountUpLatchBlockProcessor {
    ...
    public void _process(Object _p) {
        ...
        try {
            ...
            LOG.info("[WB] udate: " + updateDate + ", pdate: " +
                    DateUtils.getTimeStr(status.getCreatedAt())
                    + ",mid: " + (weibo != null ? weibo.getString("mid") : " ")
                    + ",uid: " + (user != null ? user.getString("uid") : " ")
                    + ",src_mid: " + (srcWeibo != null ? srcWeibo.getString("mid") : " ")
                    + ",src_uid: " + (srcUser != null ? srcUser.getString("uid") : " ")
            );

            if (weibo != null && user != null) {
                user.put("last_tweet_date", weibo.getString("publish_date"));
                BanyanTypeUtil.safePut(weibo, "taskId", taskId);
                BanyanTypeUtil.safePut(user, "taskId", taskId);
            ...
            if (srcWeibo != null && srcUser != null) {
                ...
                BanyanTypeUtil.safePut(srcWeibo, "taskId", taskId);
                BanyanTypeUtil.safePut(srcUser, "taskId", taskId);
            }
    ...
    public void cleanup() {
        ...
        if (size > 0) {
            ...
            ESWriterAPI cntEsWriter = cntWriter.getEsWriter();
            if (cntEsWriter != null && cntEsWriter instanceof ESWriter) {
                ESWriter esWriter = (ESWriter) cntEsWriter;
                esWriter.closeIfIdle();
            }

            ESWriterAPI userEsWriter = userWriter.getEsWriter();
            if (userEsWriter != null && userEsWriter instanceof ESWriter) {
                ESWriter esWriter = (ESWriter) userEsWriter;
                esWriter.closeIfIdle();
            }
        }
