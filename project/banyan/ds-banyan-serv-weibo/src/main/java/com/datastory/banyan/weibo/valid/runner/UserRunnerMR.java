package com.datastory.banyan.weibo.valid.runner;
...
/**
 * Created by zhaozhen on 17/7/24.
 */
public class UserRunnerMR implements Serializable {
    static final byte[] R = "r".getBytes();

    static Log LOG = LogFactory.getLog(UserRunnerMR.class);
...
//        conf.set("mapred.reduce.slowstart.completed.maps", "0.8");  // map跑完80% 才跑reducer
    ...
    public static class FixMapper extends TableMapper<NullWritable, NullWritable> {
        ...
        protected void setup(Context context) throws IOException, InterruptedException {
            ...
            projectName = "user_content";
            maxValidResultSize = 100;

            InputStream inputStream = ClassUtil.getResourceAsInputStream("weibo-valid-rule-conf.xml");
            try {
        ...
        protected void map(ImmutableBytesWritable key, Result result, Context context) throws IOException, InterruptedException {
            ...
            try {
                ...
                ValidResult validResult = engine.execute(map, "user");
    ...
    public static void main(String[] args) {
        try {
            String table = Tables.table(Tables.PH_WBUSER_TBL);
            LOG.info("[TABLE NAME]:" + table);
            new UserRunnerMR().run(table);
        } catch (Exception e) {
            LOG.error(e.getMessage(), e);
        }
    }
