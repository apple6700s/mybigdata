...
/**
 * com.datastory.banyan.wechat.valid.runner.MPRunnerMR
 *
 * @author zhaozhen
 * @since 2017/7/24
 */
public class MPRunnerMR implements Serializable {

    static final byte[] R = "r".getBytes();

    static Log LOG = LogFactory.getLog(MPRunnerMR.class);
    ...
    public void run(String table) throws Exception {
        ...
        TableMapReduceUtil.initTableMapperJob(table, scan, MPRunnerMR.FixMapper.class, NullWritable.class, NullWritable.class, job);
...
//        conf.set("mapred.reduce.slowstart.completed.maps", "0.8");  // map跑完80% 才跑reducer
    ...
    public static class FixMapper extends TableMapper<NullWritable, NullWritable> {
        ...
        protected void cleanup(Context context) throws IOException, InterruptedException {
            ...
            context.getCounter(MPRunnerMR.ROW.WRITE_REDIS_TIME).increment(cost);
        }

        @Override
        protected void map(ImmutableBytesWritable key, Result result, Context context) throws IOException, InterruptedException {
            context.getCounter(MPRunnerMR.ROW.READ).increment(1);

            try {
                long start = System.currentTimeMillis();
                Params map = new ResultRDocMapper(result).map();
                long cost = (System.currentTimeMillis() - start);
                context.getCounter(MPRunnerMR.ROW.PARSE_TIME).increment(cost);
                LOG.info("[Params]:" + map.toJson());

                start = System.currentTimeMillis();
                ValidResult validResult = engine.execute(map, "mp");
                cost = (System.currentTimeMillis() - start);
                context.getCounter(MPRunnerMR.ROW.VALID_TIME).increment(cost);
                LOG.info("[VALIDREDULT]:" + validResult.toString());
                start = System.currentTimeMillis();
                statsResult.add(validResult);
                cost = (System.currentTimeMillis() - start);
                context.getCounter(MPRunnerMR.ROW.STATS_TIME).increment(cost);
                LOG.info("[STATSRESULT]:" + validResult.toString());

            } catch (Exception e) {
                context.getCounter(MPRunnerMR.ROW.ERROR).increment(1);
                LOG.error(e.getMessage(), e);
            }
        }
