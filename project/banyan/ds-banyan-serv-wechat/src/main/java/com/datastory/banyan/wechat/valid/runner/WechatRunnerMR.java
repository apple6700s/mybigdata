 ...
 */
public class WechatRunnerMR implements Serializable {
    static final byte[] R = "r".getBytes();

    static Log LOG = LogFactory.getLog(WechatRunnerMR.class);
...
//        conf.set("mapred.reduce.slowstart.completed.maps", "0.8");  // map跑完80% 才跑reducer
    ...
    public static class FixMapper extends TableMapper<NullWritable, NullWritable> {
        ...
        protected void map(ImmutableBytesWritable key, Result result, Context context) throws IOException, InterruptedException {
            ...
            try {
                ...
                ValidResult validResult = engine.execute(map, "wechat");
    ...
    public static void main(String[] args) {
        try {
            String table = Tables.table(Tables.PH_WXCNT_TBL);
            LOG.info("[TABLE NAME]:" + table);
            new WechatRunnerMR().run(table);
        } catch (Exception e) {
            LOG.error(e.getMessage(), e);
        }
    }
