...
import com.datastory.commons3.es.bulk_writer.transport.support.DsTransportProxyClient;
 ...
 * or more nodes directly by adding their respective addresses using {@link #addTransportAddress(TransportAddress)}.
 * <p>
 * The transport client important modules used is the {@link TransportModule} which is
 * started in client mode (only connects, no bind).
 */
public class DsTransportClient extends AbstractClient {

    /**
     * Handy method ot create a {@link DsTransportClient.Builder}.
     */
    ...
    public static class Builder {
         ...
         */
        public DsTransportClient build() {
            ...
            try {
                ...
                modules.add(new DsClientTransportModule());
                ...
                DsTransportClient transportClient = new DsTransportClient(injector);
                success = true;
    ...
    private final DsTransportProxyClient proxy;

    private final DynamicNodesService dynamicNodesService;

    private DsTransportClient(Injector injector) {
        super(injector.getInstance(Settings.class), injector.getInstance(ThreadPool.class), injector.getInstance(Headers.class));
        this.injector = injector;
        nodesService = injector.getInstance(DsTransportClientNodesService.class);
        proxy = injector.getInstance(DsTransportProxyClient.class);

        Settings settings = injector.getInstance(Settings.class);
        if (settings.getAsBoolean(DynamicNodesService.SETTING_DYNAMIC_ENABLED, false)) {
            dynamicNodesService = new DynamicNodesService(this, settings);
        } else {
            dynamicNodesService = null;
        }
    }
     ...
     */
    public DsTransportClient addTransportAddress(TransportAddress transportAddress) {
     ...
     * In order to get the list of all the current connected nodes, please see {@link #connectedNodes()}.
     */
    public DsTransportClient addTransportAddresses(TransportAddress... transportAddress) {
        nodesService.addTransportAddresses(transportAddress);
        return this;
    }
     ...
     */
    public DsTransportClient removeTransportAddress(TransportAddress transportAddress) {
    ...
    @Override
    public void close() {
        injector.getInstance(DsTransportClientNodesService.class).close();
    ...
    public DynamicNodesService getDynamicNodesService() {
        return dynamicNodesService;
    }
}
