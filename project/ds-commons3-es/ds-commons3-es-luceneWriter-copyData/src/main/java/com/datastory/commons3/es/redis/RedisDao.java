package com.datastory.commons3.es.redis;
...
public class RedisDao implements Serializable {
    ...
    private static String serversInfo = null;
    private static int maxTryTimes = REDIS_TIMEOUT_MAXTRYTIMES;

    private static Map<String, JedisPool> jedisPoolMap = new ConcurrentHashMap<String, JedisPool>();
    private Pattern keyPattern;

    private RedisDao(Configuration conf) {
        this(conf, null);
    }

    private RedisDao(Configuration conf, JedisPoolConfig jedisPoolConfig) {

        serversInfo = conf.get(RedisConsts.PARAM_REDIS_SERVER);
//        serversInfo = "banyan1:etl:$dev2:6379:0";
        if (serversInfo == null) {
        ...
        for (String str : splits) {
            try {
                ...
                JedisPool jedisPool;
                if (inSps.length > 2) {
                    jedisPool = new JedisPool(jedisPoolConfig, inSps[0], Integer.parseInt(inSps[1]), redisConnectLatency, null, Integer.parseInt(inSps[2]));
                } else if (inSps.length > 3) {
                    jedisPool = new JedisPool(jedisPoolConfig, inSps[0], Integer.parseInt(inSps[1]), redisConnectLatency, inSps[3], Integer.parseInt(inSps[2]));
                } else {
                    jedisPool = new JedisPool(jedisPoolConfig, inSps[0], Integer.parseInt(inSps[1]), redisConnectLatency);
                }
    ...
    private void getRedisServers(Configuration conf) {
   ...
   /* public Pair<Pipeline, Pair<JedisPool, Jedis>> pipeline(String key) {
    ...
    public Long persist(String key) throws Exception {
        ...
        while (tryTimes-- > 0) {
            try {
                ...
                if (e.getMessage().contains("SocketTimeoutException") && tryTimes > 0) {
                    continue;
                }
     ...
     * @throws Exception
     */
    public Map<String, Response<Boolean>> existsPipeline(final List<String> keys) throws Exception {
    ...
    public boolean[] sismemberPipeline(String key, String... members) throws Exception {
        JedisPool pool = getJedisPool(key);
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
...
//        return null;
    ...
    public Long zadd(final byte[] key, final double score, final byte[] members) throws Exception {
    ...
    public Long zrem(final byte[] key, final byte[]... members) throws Exception {
        JedisPool pool = getJedisPool(new String(key));
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<byte[]> zrange(final byte[] key, final int start, final int end) throws Exception {
        JedisPool pool = getJedisPool(new String(key));
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<byte[]> zrangeByScore(final byte[] key, final double min, final double max) throws Exception {
        JedisPool pool = getJedisPool(new String(key));
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<byte[]> zrangeByScore(final byte[] key, final double min, final double max, final int offset, final int count) throws Exception {
        JedisPool pool = getJedisPool(new String(key));
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<byte[]> zrevrangeByScore(final byte[] key, final double min, final double max) throws Exception {
        JedisPool pool = getJedisPool(new String(key));
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<byte[]> zrevrange(final byte[] key, final int start, final int end) throws Exception {
        JedisPool pool = getJedisPool(new String(key));
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<String> zrangeByScore(final String key, final double min, final double max, final int offset, final int count) throws Exception {
        JedisPool pool = getJedisPool(key);
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<String> zrevrangeByScore(final String key, final double min, final double max) throws Exception {
        JedisPool pool = getJedisPool(key);
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<String> zrevrangeByScore(final String key, final double min, final double max, final int offset, final int count) throws Exception {
        JedisPool pool = getJedisPool(key);
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Set<byte[]> zrevrangeByScore(final byte[] key, final double min, final double max, final int offset, final int count) throws Exception {
        JedisPool pool = getJedisPool(new String(key));
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
    ...
    public Double[] zscorePipeline(String key, String... members) throws Exception {
        JedisPool pool = getJedisPool(key);
        if (pool == null) return null;
        Jedis jedis = pool.getResource();
...
//    }
    ...
    //For test
    public long smove(final String source, final String destination, final String value) throws Exception {
