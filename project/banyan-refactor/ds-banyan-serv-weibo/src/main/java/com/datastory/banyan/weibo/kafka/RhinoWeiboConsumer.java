...
/**
 * com.datastory.banyan.weibo.kafka.RhinoWeiboConsumer
 *
 * @author lhfcws
 * @since 16/11/23
 */

public class RhinoWeiboConsumer extends IKafkaReader {
    @Override
    ...
    protected void consume(JavaPairRDD<String, String> javaPairRDD) {
        javaPairRDD.foreachPartition(new VoidFunction<Iterator<Tuple2<String, String>>>() {
            ...
            public void call(Iterator<Tuple2<String, String>> tuple2Iterator) throws Exception {
                ...
                int size = 0;
                BlockProcessor blockProcessor = initBlockProcessor(latch);
                while (tuple2Iterator.hasNext()) {
                    try {
                        ...
                        if (needProgress) {
                            progresses.inc(AsyncTaskMonitor.getTaskId(jsonObject));
                        }
